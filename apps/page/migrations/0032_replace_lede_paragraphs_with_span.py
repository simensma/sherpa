...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for c in orm['page.Content'].objects.filter(content__contains="p class=\"lede").all():
            c.content = re.sub(r'<p class="lede">(.*?)</p>', r'<p><span class="lede">\1</span></p>', c.content, flags=re.DOTALL)
            c.save()
