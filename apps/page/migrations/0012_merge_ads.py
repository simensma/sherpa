...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for ad in orm.Ad.objects.all():
            placements = orm.AdPlacement.objects.filter(ad=ad).order_by('-end_date', '-start_date')
            if len(placements) <= 1:
                continue

            first, rest = placements[0], placements[1:]
            for p in rest:
                first.views += p.views
                first.clicks += p.clicks
                first.save()
                p.delete()

    def backwards(self, orm):
        print("Warning: Can't reverse ad-placement merge; ignoring.")
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 9, 12, 13, 37, 40, 262353)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 9, 12, 13, 37, 40, 262206)'}),
