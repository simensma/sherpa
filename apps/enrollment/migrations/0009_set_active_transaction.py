...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for enrollment in orm['enrollment.Enrollment'].objects.all():
            register_transactions = enrollment.transactions.filter(state='register')
            if register_transactions.count() != 0:
                # The active transaction was previously defined by being newest, so save
                # the newest one as active
                active_transaction = register_transactions.order_by('-initiated')[0]
    ...
    models = {
        ...
        u'enrollment.enrollment': {
            ...
            'forening': ('django.db.models.fields.IntegerField', [], {'null': 'True'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
