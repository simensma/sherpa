...
def list(request):
    ads = Ad.on(request.active_forening.get_main_site()).all().order_by('name')
    time_placements = AdPlacement.on(request.active_forening.get_main_site()).filter(start_date__isnull=False).order_by('start_date', 'end_date')
    view_placements = AdPlacement.on(request.active_forening.get_main_site()).filter(view_limit__isnull=False).order_by('views')
...
def update_ad(request):
    try:
        ad = Ad.on(request.active_forening.get_main_site()).get(id=request.POST['id'])
...
def create_placement(request):
    try:
        ad = Ad.on(request.active_forening.get_main_site()).get(id=request.POST['ad'])
...
def update_placement(request):
    try:
        placement = AdPlacement.on(request.active_forening.get_main_site()).get(id=request.POST['id'])
        placement.ad = Ad.on(request.active_forening.get_main_site()).get(id=request.POST['ad'])
