...
def searches(request):
    ...
    if most_searched is None:
        searches = Search.on(request.active_forening.get_main_site()).all()
        hashes = {}
    ...
    latest_searches = Search.on(request.active_forening.get_main_site()).all().order_by('-date')[:50]
...
def not_found(request):
    ...
    if most_requested is None:
        hits = NotFound.on(request.active_forening.get_main_site()).all()
        hashes = {}
    ...
    latest_requests = NotFound.on(request.active_forening.get_main_site()).all().order_by('-date')[:50]
