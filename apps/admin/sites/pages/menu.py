...
def new(request):
    ...
    max_order = Menu.on(request.active_forening.site).aggregate(Max('order'))['order__max']
    if max_order is None:
        max_order = 0
    menu = Menu(name=request.POST['name'], url=request.POST['url'], order=(max_order + 1), site=request.active_forening.site)
    menu.save()
...
def edit(request):
    ...
    menu = Menu.on(request.active_forening.site).get(id=request.POST['id'])
...
def reorder(request):
    for menu in json.loads(request.POST['menus']):
        obj = Menu.on(request.active_forening.site).get(id=menu['id'])
