...
def edit(request):
    ...
    menu = Menu.on(request.session['active_association'].site).get(id=request.POST['id'])
...
def delete(request):
    Menu.on(request.session['active_association'].site).get(id=request.POST['menu']).delete()
    cache.delete('main.menu')
