...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for user in orm['user.User'].objects.filter(turledere__isnull=False):
            # Just add all associations; adding the same one multiple times is just ignored by the db
            for turleder in user.turledere.all():
                association = turleder.association_approved
                # Skip central associations
                if association.id == 56 or association.id == 60 or association.id == 1180:
                    continue
                user.turleder_active_associations.add(association)
    ...
    complete_apps = ['association', 'user']
    symmetrical = True
