...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for u in orm['user.User'].objects.filter(turledere__isnull=False).distinct():
            sets = {}
            for t in u.turledere.all():
                if not t.role in sets:
                    sets[t.role] = [t]
                else:
                    sets[t.role].append(t)
            for roles in sets.values():
                if len(roles) > 1:
                    # Here's a set with multiple of the same certificate - let's try to clean up.
                    if all([r.date_end is None for r in roles]):
                        pick = roles[0]
                    else:
                        pick = max([r for r in roles if r.date_end is not None], key=lambda r: r.date_end)
                    for role in roles:
                        if role != pick:
                            role.delete()
