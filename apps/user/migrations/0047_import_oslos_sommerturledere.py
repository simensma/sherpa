...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        def save_turleder(actor):
            ...
            if orm['user.Turleder'].objects.filter(user=user, role='sommer').exists():
                return
        ...
        # Save this explicit one
        actor = orm['focus.Actor'].objects.get(memberid=1168266)
        save_turleder(actor)

        with open('%s/0047_sommerturledere.txt' % os.path.dirname(__file__)) as f:
            lines = f.readlines()

        for line in lines:
            line = line.strip('\n').decode('iso-8859-1')
            _, name, phone1, phone2, phone3, postal = line.split('\t')
            ...
            try:
                ...
                try:
                    ...
                    # Special cases
                    actor = None
                    if first_name == 'Frode' and last_name == 'Barstad':
                        actor = orm['focus.Actor'].objects.get(memberid=1044625)
                    elif first_name == 'Rasmus' and last_name == 'Ramstad':
                        actor = orm['focus.Actor'].objects.get(memberid=4322688)
                    elif first_name == 'Erik' and last_name == 'Ulven':
                        actor = orm['focus.Actor'].objects.get(memberid=1062776)
            ...
            if first_name == 'Noraker' and last_name == 'Olav':
                save_turleder(orm['focus.Actor'].objects.get(memberid=4205473))
            ...
            print("%s, %s#%s#%s#%s#%s" % (last_name, first_name, phone1, phone2, phone3, postal))
