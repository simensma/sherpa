...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for image in orm['aktiviteter.AktivitetImage'].objects.all():
            if s3_bucket(ssl=False, ignore_debug=True) in image.url:
                image.url = re.sub(
                    s3_bucket(ssl=False, ignore_debug=True),
                    s3_bucket(ssl=True, ignore_debug=True),
                    image.url
                )
                image.save()
    ...
    models = {
        u'aktiviteter.aktivitet': {
            ...
            'omrader': ('djorm_pgarray.fields.TextArrayField', [], {'default': '[]', u'dbtype': "u'text'", 'blank': 'True'}),
