...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for site in orm['core.Site'].objects.filter(template='main'):
            site.template = 'central'
            site.save()
        for site in orm['core.Site'].objects.filter(template='large'):
            site.template = 'local'
            site.save()

    def backwards(self, orm):
        for site in orm['core.Site'].objects.filter(template='central'):
            site.template = 'main'
            site.save()
        for site in orm['core.Site'].objects.filter(template='local'):
            site.template = 'large'
            site.save()
