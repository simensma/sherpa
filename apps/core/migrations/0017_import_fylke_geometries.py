...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Exclude fylke 7 and 8 (duplications of Sør-Trøndelag with weird data)
        for fylke in orm['sherpa2.Fylker'].objects.exclude(pk__in=[7, 8]):
            if fylke.pk == 1:
                # Special case for Finnmark which has a weird name
                county = orm['core.County'].objects.get(code='20')
            else:
                county = orm['core.County'].objects.get(name=fylke.navn)

            county.area = fylke.area
            county.perimeter = fylke.perimeter
            county.geom = fylke.the_geom
            county.save()
