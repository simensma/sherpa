class Router(object):
    def db_for_read(self, model, **hints):
        if model._meta.object_name == 'Group':
            return 'sherpa-2'
        elif model._meta.object_name in ['FocusUser', 'FocusActType', 'Actor', 'ActorAddress', 'FocusZipcode', 'FocusPrice']:
            return 'focus'
        return None
    ...
    def allow_syncdb(self, db, model):
        if model._meta.object_name in ['Group', 'FocusUser', 'FocusActType', 'Actor', 'ActorAddress', 'FocusZipcode', 'FocusPrice']:
            return False
